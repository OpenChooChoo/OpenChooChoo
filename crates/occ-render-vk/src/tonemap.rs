...
const TONEMAP_VERT_SPV: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/tonemap.vert.spv"));
const TONEMAP_FRAG_SPV: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/tonemap.frag.spv"));
...
impl TonemapPipeline {
    pub(crate) fn new(
        ...
        let vert = create_module(device, TONEMAP_VERT_SPV)?;
        device.set_debug_name(vert, "Tonemap Vertex SPV");
        let frag = create_module(device, TONEMAP_FRAG_SPV)?;
        device.set_debug_name(frag, "Tonemap Fragment SPV");
        ...
        let layout = unsafe { device.device.create_pipeline_layout(&layout_info, None) }
            .context("tonemap pipeline layout")?;
        device.set_debug_name(layout, "Tonemap Pipeline Layout");
        ...
        .map_err(|(_, err)| anyhow::anyhow!("tonemap pipeline creation failed: {err:?}"))?;
        let pipeline = pipelines[0];
        device.set_debug_name(pipeline, "Tonemap Pipeline");
...
fn create_module(device: &Device, bytes: &[u8]) -> anyhow::Result<vk::ShaderModule> {
    ...
    let module = unsafe { device.device.create_shader_module(&info, None) }
        .context("failed to create tonemap shader module")?;
    Ok(module)
}
